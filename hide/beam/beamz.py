...
#		valid_i, valid_j = i,j#i[valid_idxs], j[valid_idxs]
		...
		if plot:
			...
			if fits_writing:
				import astropy.io.fits as pyfits
				fitsfile = "/home/joao/Documentos/cosmologia/hide_and_seek/resultados/hide_beams/MINUS990_304/nside{nside}/healpix_MINUS990_304_beam_nside{nside}_{interp}.fits"
				# 30_MINUS304
...
#			hp.graticule(local=True) # default interval = 1 deg
			...
			if fits_writing:
				pyfits.writeto(fitsfile.format(nside=params.beam_nside,interp="nearest"), 
							   mapa,overwrite=True)
			...
			if fits_writing:
				pyfits.writeto(fitsfile.format(nside=params.beam_nside,interp="linear"),
							   mapa,overwrite=True)
